...
export default function SignalGapVisualizer({ initialCases }: SignalGapVisualizerProps) {
  const [signalCases, setSignalCases] = useState<SignalCase[]>(initialCases || []);
  const [isLoadingCases, setIsLoadingCases] = useState<boolean>(!initialCases || initialCases.length === 0);
  ...
  useEffect(() => {
    ...
    async function loadSignalCases() {
      if (initialCases && initialCases.length > 0) {
        setSignalCases(initialCases);
      ...
      try {
        ...
        if (res.ok) {
          ...
          const rawList = data.markets || data.data || [];
          if (isMounted && Array.isArray(rawList) && rawList.length > 0) {
            const mapped: SignalCase[] = rawList.map((m: any) => {
              const agreeP = Number(m.agreePool ?? m.agree_pool ?? 0);
              const disagreeP = Number(m.disagreePool ?? m.disagree_pool ?? 0);
              const totalEth = Number((agreeP + disagreeP).toFixed(2));
              const moneyAgreePct = totalEth > 0 ? Math.round((agreeP / totalEth) * 100) : 50;
              const moneyDisagreePct = 100 - moneyAgreePct;

              const agreeV = Number(m.agreeParticipants ?? m.agree_participants ?? 0);
              const disagreeV = Number(m.disagreeParticipants ?? m.disagree_participants ?? 0);
              const totalVotes = agreeV + disagreeV;
              const peopleAgreePct = totalVotes > 0 ? Math.round((agreeV / totalVotes) * 100) : (m.agreePercentage ? Math.round(m.agreePercentage) : 50);
              const peopleDisagreePct = 100 - peopleAgreePct;

              const gap = Math.abs(peopleAgreePct - moneyAgreePct);
              const gapType = gap > 15 ? (moneyAgreePct > peopleAgreePct ? "smart-money" : "overhyped") : "aligned";
              const gapBadge = gap > 15 ? (moneyAgreePct > peopleAgreePct ? `+${gap}% Smart Money Accumulation` : `+${gap}% Retail Hype Divergence`) : "Balanced Consensus Equilibrium";

              const author = m.author || "Analyst";
              const authorHandle = m.authorHandle || (author.startsWith("@") ? author : `@${author.replace(/\s+/g, '')}`);
              const authorAvatar = author.slice(0, 2).toUpperCase() || "AN";
              const category = m.category || "Prediction";
              const statement = m.statement || m.title || "Market Belief";

              const analysisText = moneyAgreePct > peopleAgreePct
                ? "Social commentators remain cautious, yet on-chain capital is actively positioning on the Agree side at favorable odds."
                : peopleAgreePct > moneyAgreePct
                ? "Crowd conviction is noticeably bullish on social channels, while capital pool staking shows defensive Disagree hedging."
                : "Retail sentiment and institutional capital staking are in equilibrium with minimal spread.";
              ...
              return {
                ...
                peopleAgreePct,
                peopleDisagreePct,
                peopleTotal: totalVotes > 0 ? totalVotes : 100,
                moneyAgreePct,
                moneyDisagreePct,
                moneyTotalEth: totalEth,
                gapPct: gap,
                gapType,
                gapBadge,
                analysisText,
            ...
            setSignalCases(mapped);
            setSelectedCaseId((prev) => prev || mapped[0]?.id || "");
          }
        }
      } catch {
      } finally {
  ...
  const currentCase = signalCases.find((c) => c.id === selectedCaseId) || signalCases[0] || null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        <div className="lg:col-span-7 flex flex-col h-full">
          {signalCases.length > 0 && (
            <div className="h-10 flex items-center gap-2 overflow-x-auto pb-1 mb-4 scrollbar-none shrink-0">
              {signalCases.map((item) => {
                const isActive = item.id === (currentCase?.id || selectedCaseId);
                return (
                  <button
                    key={item.id}
                    type="button"
          ...
          <div className="p-6 sm:p-7 rounded-3xl border transition-all duration-300 hover-lift shadow-xl flex-1 flex flex-col justify-between bg-white/95 dark:bg-[#070D09]/95 border-emerald-500/15 dark:border-emerald-500/20 shadow-[0_12px_32px_rgba(14,122,78,0.06)] dark:shadow-[0_16px_40px_rgba(0,0,0,0.7)] text-[#0B1F16] dark:text-white">
            {isLoadingCases ? (
              <div className="p-12 text-center rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800 animate-pulse font-mono text-xs text-zinc-500 my-auto">
                Loading consensus market signal gaps...
              </div>
            ) : currentCase ? (
              ...
              <div className="p-12 text-center rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800 font-mono text-xs text-zinc-500 my-auto">
                No active signal gap data found.
              </div>
            )}
          </div>
        </div>
