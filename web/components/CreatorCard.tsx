    ...
    >
      ...
      <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800/80 flex items-center gap-2">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center justify-center gap-1.5 py-2.5 px-3.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 text-xs sm:text-sm font-bold font-mono transition-all shadow-2xs shrink-0"
          title={`Open @${(creator.handle || creator.name).replace('@', '')} on X`}
          aria-label={`Open @${(creator.handle || creator.name).replace('@', '')} on X`}
        >
