...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'belief_id') THEN
    ALTER TABLE markets ADD COLUMN belief_id UUID REFERENCES beliefs(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'contract_address') THEN
    ALTER TABLE markets ADD COLUMN contract_address TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'chain_id') THEN
    ALTER TABLE markets ADD COLUMN chain_id INTEGER DEFAULT 11155111;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'agree_pool') THEN
    ALTER TABLE markets ADD COLUMN agree_pool NUMERIC DEFAULT 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'disagree_pool') THEN
    ALTER TABLE markets ADD COLUMN disagree_pool NUMERIC DEFAULT 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'open_time') THEN
    ALTER TABLE markets ADD COLUMN open_time TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'close_time') THEN
    ALTER TABLE markets ADD COLUMN close_time TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'resolution_type') THEN
    ALTER TABLE markets ADD COLUMN resolution_type TEXT CHECK (resolution_type IN ('PRICE_ABOVE', 'PRICE_BELOW', 'RELATIVE_PERFORMANCE'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'resolution_config') THEN
    ALTER TABLE markets ADD COLUMN resolution_config JSONB;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'metadata_hash') THEN
    ALTER TABLE markets ADD COLUMN metadata_hash TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'markets' AND column_name = 'winner') THEN
    ALTER TABLE markets ADD COLUMN winner TEXT CHECK (winner IN ('AGREE', 'DISAGREE', 'VOID'));
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS oracle_snapshots (
  ...
  snapshot_type TEXT CHECK (snapshot_type IN ('START', 'END', 'DISPLAY')),
